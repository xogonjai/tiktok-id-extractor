...
Supports URLs like `https://www.tiktok.com/t/ZP8kqsafJ/`.  
...
short_url = st.text_input("TikTok Shop URL:", placeholder="e.g., https://www.tiktok.com/t/ZP8kqsafJ/", value="https://www.tiktok.com/t/ZP8kqsafJ/", key="url_input")
...
def extract_and_fill_tiktok_ids(short_url, checkout_url_template):
    ...
    try:
        ...
        # Fallback to provided Product ID
        if not product_id:
            product_id = "1729543202963821377"
        ...
        if not seller_id:
            ...
            for pattern in seller_id_patterns:
                ...
                if seller_id_match:
                    ...
                    break
            # Check if 7495316114727995400 is in the page source
            if not seller_id and "7495316114727995400" in text:
                seller_id = "7495316114727995400"
                st.info("**Note**: Seller ID 7495316114727995400 found in page source and used.")
        ...
        if seller_id:
            ...
            if seller_id == "7495316114727995400":
                st.warning("**Warning**: Seller ID 7495316114727995400 was used. Verify it works in checkout, as it caused errors previously.")
        else:
        ...
        if product_id:
            ...
            st.warning("**Product ID**: Not found in URL or page source. Using provided ID 1729543202963821377.")
        ...
        if sku_ids:
            ...
            st.warning("**SKU ID**: Not found in page source or URL. Using provided ID 1731062885389669185.")
        ...
        **Try these steps**:
        ...
        4. Contact the seller via the TikTok app or Seller Support (https://t.co/bm1rH6eW7N).
        """)
        if product_id:
            st.write(f"**Product ID**: {product_id}")
        ...
        if sku_id_url:
            st.write(f"**SKU ID (from URL)**: {sku_id_url}")
        return product_id, [sku_id_url] if sku_id_url else ["1731062885389669185"], [partial_url] if product_id else [], [], None
...
1. **Resolve Product URL**:
   - Open the URL (e.g., https://www.tiktok.com/t/ZP8kqsafJ/) in Chrome or Safari on your phone.
   - Note the full URL after redirect (e.g., https://www.tiktok.com/view/product/1729543202963821377?...).
2. **Find Product ID**:
...
3. **Find SKU ID and Seller ID for Each Variant**:
   ...
   - Copy the checkout URL, e.g., `sku_id=1731062885389669185&product_id=1729543202963821377&seller_id=7495316114727995400`.
4. **Fill Checkout URL**:
...
5. **View Page Source (Alternative)**:
   - Type `view-source:[full URL]` in your browser.
   - Search for `sku_id"\s*:\s*"\d+"`, `seller_id"\s*:\s*"\d+"`, `sellerId"\s*:\s*"\d+"`, or `shop_id"\s*:\s*"\d+"`.
6. **Contact Seller**:
   - Message the seller via the TikTok app or use Seller Support (https://t.co/bm1rH6eW7N) to confirm the **Seller ID**.
""")
